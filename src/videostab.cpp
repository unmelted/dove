...
Mat VideoStab::stabilize(Mat frame_1, Mat frame_2)
{
    ...
    goodFeaturesToTrack(frame2, features2, 200, 0.01  , 30 );
    ...
    Mat T = getRotationMatrix2D(Point2f(smoothedFrame.cols/2, smoothedFrame.rows/2), 0, 1.04);
    warpAffine(smoothedFrame, smoothedFrame, T, smoothedFrame.size());
